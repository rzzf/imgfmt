...
const runtimeEntryRequest = "imgfmt:runtime.js";
const runtimeEntryOutput = "imgfmt-runtime";
const runtimeNamespace = "imgfmt-runtime";
const runtimeMetafileEntry = `${runtimeNamespace}:runtime.js`;
...
export function setupEsbuild(build: PluginBuild, inputOptions: ImgfmtOptions = {}): void {
  ...
  initialOptions.entryPoints = appendRuntimeEntry(initialOptions.entryPoints);

  build.onStart(() => {
    transformedCss.clear();
  });

  build.onResolve({ filter: /^imgfmt:runtime\.js$/ }, (args) => {
  ...
  build.onLoad({ filter: /.*/, namespace: runtimeNamespace }, () => ({
    contents: runtimeSource,
    loader: "copy",
    watchFiles: [...new Set(watchedDocuments)],
  }));
  ...
  build.onEnd(async (result): Promise<OnEndResult | undefined> => {
    ...
    try {
      ...
      if (metafile === undefined) {
        throw new Error("imgfmt/esbuild requires build metadata to locate its runtime asset");
      }
      ...
      const runtimeFileName = findRuntimeFileName(metafile, root, outputDirectory);
      const manifest = createManifest(runtimeFileName, initialOptions.publicPath);
...
function appendRuntimeEntry(
  entryPoints: BuildOptions["entryPoints"],
): NonNullable<BuildOptions["entryPoints"]> {
  if (entryPoints === undefined) {
    throw new TypeError("imgfmt/esbuild requires at least one application entry point");
  }
  ...
  if (Array.isArray(entryPoints)) {
    if (
      entryPoints.some((entry) => typeof entry === "object" && entry.out === runtimeEntryOutput)
    ) {
      throw new TypeError(`imgfmt/esbuild owns the ${runtimeEntryOutput} entry name`);
    }

    return [
      ...entryPoints,
      {
        in: runtimeEntryRequest,
        out: runtimeEntryOutput,
      },
    ];
  }

  if (Object.hasOwn(entryPoints, runtimeEntryOutput)) {
    throw new TypeError(`imgfmt/esbuild owns the ${runtimeEntryOutput} entry name`);
  }

  return {
    ...entryPoints,
    [runtimeEntryOutput]: runtimeEntryRequest,
  };
}
...
function findRuntimeFileName(metafile: Metafile, root: string, outputDirectory: string): string {
  const matches = Object.entries(metafile.outputs).filter(
    ([, output]) => output.entryPoint === runtimeMetafileEntry,
  );

  if (matches.length !== 1) {
    throw new Error(`imgfmt/esbuild expected one runtime output, found ${matches.length}`);
  }

  const outputPath = matches[0]?.[0];

  if (outputPath === undefined) {
    throw new Error("imgfmt/esbuild could not locate its runtime output");
  }

  const absoluteOutput = isAbsolute(outputPath) ? outputPath : resolve(root, outputPath);
  const relativeOutput = normalizePath(
    relative(realPathOrResolved(outputDirectory), realPathOrResolved(absoluteOutput)),
  );
  ...
  if (
    ...
    !relativeOutput.toLowerCase().endsWith(".js")
  ) {
    throw new Error("imgfmt/esbuild runtime output escaped the configured outdir");
  }
...
function createManifest(
  ...
  const runtimeUrlFor = (htmlOutput: string): string => {
    if (
      ...
      return relativeRuntimeUrl(htmlOutput, runtimeFileName);
    }

    if (/^(?:[a-z][a-z0-9+.-]*:|\/\/|\/)/i.test(publicPath)) {
      return joinPublicPath(publicPath, runtimeFileName);
    }

    throw new TypeError(
      'imgfmt/esbuild requires publicPath to be absolute, root-relative, or "./" when it owns HTML',
    );
  };

  return Object.freeze({
    install(html: string, htmlOutput: string): string {
      return injectDocumentBootstrap(html, runtimeUrlFor(htmlOutput));
    },
    runtimeFileName,
    runtimeUrlFor,
  });
...
async function writeStaticDocuments(
  ...
  const outputs = await Promise.all(
    documents.map(async (document) => ({
      contents: manifest.install(await readFile(document.input, "utf8"), document.output),
