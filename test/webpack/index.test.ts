...
describe("imgfmt/webpack runtime", () => {
  it("builds source CSS variants and installs one inline runtime in nested HTML", async () => {
    ...
    try {
      ...
      expect(html.indexOf(runtimeTag.tag)).toBeLessThan(html.indexOf('src="../assets/main.'));
...
function assertShorthandMirrorsOnlyItsImage(css: string): void {
  ...
  const original = stylesheet.nodes.find(
    (node): node is Rule => node.type === "rule" && node.selector === ".banner",
  );
