  ...
  it("replaces a local path extension while retaining query and fragment suffixes", () => {
    expect(
      resolveSiblingVariantUrl({
        extension: ".avif",
        format: "avif",
        originalUrl: "/images/product-card.large.png?v=1#crop",
      }),
    ).toBe("/images/product-card.large.avif?v=1#crop");
  });
  ...
  it("appends to dotfiles instead of replacing the whole filename", () => {
    expect(
      resolveSiblingVariantUrl({
        extension: ".webp",
        format: "webp",
        originalUrl: "/images/.banner?theme=dark",
      }),
    ).toBe("/images/.banner.webp?theme=dark");
  });
...
describe("isSupportedSourceImageUrl", () => {
  it("accepts common raster image URLs", () => {
    for (const url of [
      "image.apng",
      "image.avif",
      "image.bmp",
      ...
      "image.jpg",
      "image.png",
      "image.WEBP?theme=dark#cover",
    ]) {
      expect(isSupportedSourceImageUrl(url)).toBe(true);
    }
  });

  it("rejects non-raster and unmanaged URLs", () => {
    for (const url of [
      "image.svg",
      "image.svg?theme=dark#cover",
      "image.tiff",
      "font.woff2",
      "asset",
      ".hidden",
      ...
      expect(isSupportedSourceImageUrl(url)).toBe(false);
    }
  });
});
